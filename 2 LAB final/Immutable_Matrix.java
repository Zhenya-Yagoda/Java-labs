...
public final class  Immutable_Matrix {
    private int n;
    private int m;
    String [][] mas;
    String [][] copymas;
    Immutable_Matrix(int n,int m){  //конструктор заданої пустої матриці
        this.n=n;
        this.m=m;
    ...
    Immutable_Matrix ( String args , int n , int m){  //копіювання матриць
        this.n=n;
        this.m=m;
        copymas=new String[this.n][this.m];
    }

    public static Immutable_Matrix copy(Immutable_Matrix mymas) {
        Immutable_Matrix my = new Immutable_Matrix(mymas.mas.length,mymas.mas[0].length);
        my=mymas;
        return my;
    }
    ...
    public static Immutable_Matrix ramdommat(int n,int m){ //матриця випадкових чисел
        Immutable_Matrix my = new Immutable_Matrix(n, m);
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                my.mas[i][j] = ranvalue();
            }
    ...
    public static Immutable_Matrix putvalue(int n, int m) { //заповнення значень в матрицю
        Immutable_Matrix my = new Immutable_Matrix(n, m);
        Scanner scanner = new Scanner(System.in);
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
    ...
    public static Immutable_Matrix changevalue(Immutable_Matrix mymas , int n, int m) { //зміна значень
        Immutable_Matrix my = new Immutable_Matrix(mymas.mas.length, mymas.mas[0].length);
        my.mas= mymas.mas;
        Scanner scanner = new Scanner(System.in);
    ...
    public static Immutable_Matrix changeValueS(Immutable_Matrix mymas , int k) { //зміна декількох значень
        Immutable_Matrix my = new Immutable_Matrix(mymas.mas.length, mymas.mas[0].length);
        my.mas= mymas.mas;
        Scanner scanner = new Scanner(System.in);
        for (int i=0 ; i<k ; i++){
    ...
    public boolean equals(Immutable_Matrix mas1) { //визначення методу equals для матриць
        Immutable_Matrix mas2 = this;
        int answer = 0;
